...
; Grey out owned Tools and make it unbuyable
; From Draw all text for Buy menu
org $C3B9C0
	JSR C3FFEE		; Check if you have a tool and grey out
 
org $C3FFEE
C3FFEE:
	LDA #$20		; User Colour
	STA $29			; Set
	JSL C9FDC0		; Check tool
	JSR $7FD9       ; Draw name
	RTS
...
C9FDC0:
	LDY $00         ; Stock slot: 1
	LDX $F1			; Item slot index
	LDA $7E9D89,X   ; Shop item
	CMP #$A3		; Tool?
	BCS .can_be		; Branch if can
	RTL				; Return if not
.can_be
	CMP #$AB		; Tool?
	BCC .loop		; Branch if so
	RTL				; Rturn if not
.loop  
	CMP $1869,Y     ; In stock slot?
    BEQ .equal		; Branch if so
	INY             ; Stock slot +1
...
C9FDE3:
	...
	LDA $4B         ; Cursor slot
	TAX             ; Index it
	LDA $7E9D89,X   ; Item owned
	CMP #$A3		; Tool?
	BCS .can_be		; Branch if can
	BRA .exit		; Return if not
.can_be
	CMP #$AB		; Tool?
	BCS .exit		; Branch if not	
	LDA #$01		; Quantity: 01
	RTL
.exit
	LDA #$63        ; Quantity: 99
...
org $C3B834
	JSL C9FDE3		; St Tool Q.ty to only 1 