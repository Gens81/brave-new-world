...
; slices into "load character portrait color palettes" routine 
org $C36C1A
    BRA Skip1

org $C36C25 : Skip1:

; slices into "load character portrait graphics" routine
org $C36E93
    BRA Skip2

org $C36E9E : Skip2:

; slices into "get pointer to character portrait graphics" routine
org $C36EDA
    BRA Skip3
...
; slice into Terra's scenario event
org $CB0997 : db $FD,$FD        ; replace event command with NOPs
org $CB0999
    db $B2                      ; jump to new event code
    dl TerraScenario-EventBase  ; ^ continued
warnpc $CB099D
...
TerraScenario:
    db $B2,$95,$CB,$00          ; [displaced] call subroutine $CACB95 (hide party members 2-4)
...
MogTutorial:
    ...
    db $4B,$7B,$02              ; display caption $27B (634) [choreography tutorial]
    db $92                      ; wait for 30 frames (1/2 second)
    db $FE                      ; return
...
; -----------------------------------------------------------------------------
; Event hack for Hell Claw backdoor
; -----------------------------------------------------------------------------

org $CA96B1
    db $B2                      ; call subroutine
    dl HellClawHook-EventBase   ; ^ continued
warnpc $CA96B5

org EventScriptFreespace_7
HellClawHook:                   ; 12 bytes
    db $B2,$A9,$5E,$00          ; [displaced] call subroutine $CA5EA9 (post-battle)
    db $B7,$4F,$B3,$5E,$00      ; if ($1DC9($04F) [$1DD2.7] is clear) return
    db $80,$57                  ; add item Hell Claw to inventory
...
org $CB05B1
    db $88,$04,$08,$40          ; cure Edgar of all status conditions but Magitek, Interceptor
    db $88,$00,$08,$40          ; cure Terra of all status conditions but Magitek, Interceptor
    db $88,$05,$08,$40          ; cure Sabin of all status conditions but Magitek, Interceptor
    db $88,$0E,$08,$40          ; cure Banon of all status conditions but Magitek, Interceptor
warnpc $CB05C1

; fixes map tile glitch in Owzer's basement (fix by Gi Nattak)
org $CB4608 : db $3A            ; enable player to move while event commands execute
